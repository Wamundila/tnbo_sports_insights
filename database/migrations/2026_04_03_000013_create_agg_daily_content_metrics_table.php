...
return new class extends Migration
{
    ...
    {
        Schema::create('agg_daily_content_metrics', function (Blueprint $table): void {
            $table->id();
            $table->date('metric_date');
            $table->string('service', 60)->index();
            $table->string('content_type', 100)->index();
            $table->string('content_id', 150)->index();
            ...
            $table->unique(['metric_date', 'service', 'content_type', 'content_id'], 'agg_daily_content_metrics_scope');
        });
    }
